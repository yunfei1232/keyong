...
def evaluate_individual(path, time_weight, cost_weight, co2_emission_weight):
    ...
    total_distance, total_cost, total_time, total_co2_emission, _, _, _, _, _, _, _,_ = calculate_total_distance_cost_and_time(path)
    ...
    # 根据权重计算综合评分
    score = (normalized_total_time * time_weight +normalized_total_cost * cost_weight +normalized_total_co2_emission * co2_emission_weight)
...
def calculate_total_distance_cost_and_time(path, start_time=11):
    ...
    for i in range(len(path) - 1):
        ...
        total_cost += distance * (air_transport_cost_per_km if mode == "air" else rail_transport_cost_per_km) * cargo_weight
...
if __name__ == "__main__":
    ...
    ga_regret_coefficient = 0.3  # GA的遗憾系数
